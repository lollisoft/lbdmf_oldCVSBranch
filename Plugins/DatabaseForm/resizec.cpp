...
*/
		if (size.y <= wxResizeableControl::SizeYRad*2) size.y = 30;
        m_child->SetSize(wxResizeableControl::SizeXRad*2,
                         wxResizeableControl::SizeYRad*2,
                         size.x-wxResizeableControl::SizeXRad*2,
                         size.y-wxResizeableControl::SizeYRad*2);
		
		wxSize newsize = m_child->GetSize();
		wxPoint newpoint = m_child->GetPosition();
		
		_LOG << "Have new size for control: x=" << newsize.x << ", y=" << newsize.y << " and position x=" << newpoint.x << ", y=" << newpoint.y LOG_
	}
...
{
	_LOG << "wxResizeableParentControl::OnPaint(wxPaintEvent &event) Have called" LOG_
    if (m_hasfocus) {
		wxPaintDC dc(this);
        DrawSizeRect(dc);
	}
