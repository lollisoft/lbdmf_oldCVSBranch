...
<xsl:template name="DBReaderExtension.h">
	<xsl:param name="ApplicationID"/>
	<xsl:param name="FormularID"/>
 ...
 */
class <xsl:value-of select="$FormName"/>DBReaderExtension : public lb_I_VisitorExtension {
public:
	<xsl:value-of select="$FormName"/>DBReaderExtension();
	virtual ~<xsl:value-of select="$FormName"/>DBReaderExtension();
...
DECLARE_FUNCTOR(instanceOf<xsl:value-of select="$FormName"/>DBReaderExtension)
</xsl:template>
</xsl:stylesheet>
