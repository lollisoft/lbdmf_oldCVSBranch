...
<xsl:template name="XMLWriterExtension.h">
	<xsl:param name="ApplicationID"/>
	<xsl:param name="FormularID"/>
 ...
 */
class <xsl:value-of select="$FormName"/>XMLWriterExtension : public lb_I_VisitorExtension {
public:
	<xsl:value-of select="$FormName"/>XMLWriterExtension();
	virtual ~<xsl:value-of select="$FormName"/>XMLWriterExtension();
...
DECLARE_FUNCTOR(instanceOf<xsl:value-of select="$FormName"/>XMLWriterExtension)
</xsl:template>
</xsl:stylesheet>
