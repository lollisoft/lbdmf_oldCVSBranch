...
<xsl:template name="InternalFormatWriterExtension.h">
	<xsl:param name="ApplicationID"/>
	<xsl:param name="FormularID"/>
 ...
 */
class <xsl:value-of select="$FormName"/>InternalFormatWriterExtension : public lb_I_VisitorExtension {
public:
	<xsl:value-of select="$FormName"/>InternalFormatWriterExtension();
	virtual ~<xsl:value-of select="$FormName"/>InternalFormatWriterExtension();
...
protected:
	...
	UAP(lb_I_<xsl:value-of select="$FormName"/>, owningObject)
};

DECLARE_FUNCTOR(instanceOf<xsl:value-of select="$FormName"/>InternalFormatWriterExtension)
</xsl:template>
</xsl:stylesheet>
