...
public:
	virtual long		LB_STDCALL add(<xsl:for-each select="//lbDMF/formularfields/formular[@formularid=$FormularID]"><xsl:variable name="FieldName" select="@name"/><xsl:variable name="TableName" select="@tablename"/><xsl:choose>
...
</xsl:for-each> long _<xsl:value-of select="$FormName"/>ID = -1) = 0;
