 ...
 */
  ...
  public:
      ...
      {
		TEST_CASE(testForeignKeysSetupPrimary)
		TEST_CASE(testForeignKeysSetupForeign)
		TEST_CASE(testForeignKeysViolate)
		TEST_CASE(testForeignKeysNULL)
		TEST_CASE(testForeignKeysSetBogus)
    ...
    private:
      ...
      DatabaseLayer* m_pDatabaseLayer2;
	  ...
	  #define CREATE_TABLE10 14 // Primary table 
	  #define CREATE_TABLE11 15 // Foreign table
	  
	  #define DROP_TABLE10 16 // Primary table 
	  #define DROP_TABLE11 17 // Foreign table
  ...
  public:
      void setUp()
      {
        ...
        if (!databaseSettingsLoaded)
        {
          ...
          return;
			...
			{
#endif
				SQL_STATEMENT_ARRAY::iterator createTableSql = m_SqlMap.find(CREATE_TABLE10);
				if (createTableSql != m_SqlMap.end())
				{
					for (unsigned int i=0; i<(*createTableSql).second.size(); i++)
					{
			...
			{
#endif
				SQL_STATEMENT_ARRAY::iterator createTableSql = m_SqlMap.find(CREATE_TABLE11);
				if (createTableSql != m_SqlMap.end())
				{
					for (unsigned int i=0; i<(*createTableSql).second.size(); i++)
					{
			...
			if (m_pDatabaseLayer) {
				...
				m_pDatabaseLayer->RunQuery(_("DELETE FROM table11;")); // Be extra sure the table is empty
				m_pDatabaseLayer->RunQuery(_("DELETE FROM table10;")); // Be extra sure the table is empty
			}
#ifndef DONT_USE_DATABASE_LAYER_EXCEPTIONS
        }
        catch (DatabaseLayerException& e)
      ...
      void tearDown()
      {
        ...
        if (m_pDatabaseLayer)
        {
          ...
          m_DeleteStatements.Clear();
			...
			{
#endif
				SQL_STATEMENT_ARRAY::iterator createTableSql = m_SqlMap.find(DROP_TABLE11);
				if (createTableSql != m_SqlMap.end())
				{
					for (unsigned int i=0; i<(*createTableSql).second.size(); i++)
					{
			...
			{
#endif
				SQL_STATEMENT_ARRAY::iterator createTableSql = m_SqlMap.find(DROP_TABLE10);
				if (createTableSql != m_SqlMap.end())
				{
					for (unsigned int i=0; i<(*createTableSql).second.size(); i++)
					{
	  ...
	  /* Test functionality for foreign keys
       */
      void testForeignKeysSetupPrimary( void )
      {
		  puts("testForeignKeysSetupPrimary");
		  ASSERT( m_pDatabaseLayer != NULL );
		  ...
		  if (m_pDatabaseLayer)
		  {
			  ...
			  try
			  {
				  ...
				  int nCount = GetRowCount("table10");
				  ASSERT_EQUALS( 0, nCount );
				  
				  // Add a new row to the primary database table
				  
				  // IntCol int PRIMARY KEY, StringCol VARCHAR(50), ForeignCol
				  
				  m_pDatabaseLayer->RunQuery(_("INSERT INTO table10 (StringCol) VALUES ('ONE');"));
				  DatabaseErrorCheck(m_pDatabaseLayer);
				  
				  // Get the updated row count
				  wxPrintf(_("Checking if row count = 1\n"));
				  nCount = GetRowCount("table10");
	  ...
	  /* Test functionality for foreign keys
	   * This function cannot run alone. The function testForeignKeysSetupPrimary needs to be run before.
	   * That is due to the setUp() and teardown() functions are called per test.
       */
      void testForeignKeysSetupForeign( void )
      {
		  puts("testForeignKeysSetupForeign");
		  ASSERT( m_pDatabaseLayer != NULL );
		  ...
		  if (m_pDatabaseLayer)
		  {
			  ...
			  try
			  {
#endif  
				  // Setup primary row
				  testForeignKeysSetupPrimary();
				  
				  // Get the initial row count
				  wxPrintf(_("Checking if row count = 0\n"));
				  int nCount = GetRowCount("table11");
				  ASSERT_EQUALS( 0, nCount );
				  ...
				  m_pDatabaseLayer->RunQuery(_("INSERT INTO table11 (StringCol, ForeignCol) VALUES ('ONE', 1);"));
				  DatabaseErrorCheck(m_pDatabaseLayer);
				  
				  // Get the updated row count
				  wxPrintf(_("Checking if row count = 1\n"));
				  nCount = GetRowCount("table11");
				  ASSERT_EQUALS( 1, nCount );
	  ...
	  void testForeignKeysViolate( void )
	  {
		  puts("testForeignKeysViolate");
		  ASSERT( m_pDatabaseLayer != NULL );
		  ...
		  if (m_pDatabaseLayer)
		  {
			  ...
			  try
			  {
				  ...
				  m_pDatabaseLayer->RunQuery(_("INSERT INTO table11 (StringCol, ForeignCol) VALUES ('ONE', 0);"));
				  DatabaseErrorCheck(m_pDatabaseLayer);
			  ...
			  {
				  if (e.GetErrorMessage() == "ForeignCol violates foreign key table10(IntCol)") {
					  // Get the updated row count
					  wxPrintf(_("Checking if row count = 0\n"));
					  int nCount = GetRowCount("table11");
					  ASSERT_EQUALS( 0, nCount );
	  ...
	  void testForeignKeysNULL( void )
	  {
		  ...
		  if (m_pDatabaseLayer)
		  {
			  ...
			  try
			  {
				  ...
				  m_pDatabaseLayer->RunQuery(_("INSERT INTO table11 (StringCol) VALUES ('ONE');"));
				  DatabaseErrorCheck(m_pDatabaseLayer);
	  ...
	  void testForeignKeysSetBogus( void )
	  {
		  puts("testForeignKeysViolate");
		  ASSERT( m_pDatabaseLayer != NULL );
		  ...
		  if (m_pDatabaseLayer)
		  {
			  ...
			  try
			  {
				  ...
				  m_pDatabaseLayer->RunQuery(_("UPDATE table11 set ForeignCol = 0 where ForeignCol = 1;"));
				  DatabaseErrorCheck(m_pDatabaseLayer);
          ...
          if (pStatement)
          {
            ...
            if (pResultSet)
            {
              ...
              if (pResultSet->Next())
              {
                ...
                ASSERT_MESSAGE( bMatch, "Retrieved Blob does NOT match original Blob" );
              }
              else
              {
                m_pDatabaseLayer->CloseResultSet(pResultSet);
            ...
            if (pResultSet)
            {
              ...
              while (pResultSet->Next())
              {
                ...
                for (int i=0; i<nDataLength && bMatch; i++)
                {
                  if (*pOriginalBuffer != *pRetrievedBuffer)
                  {
                    wxPrintf(_("Buffers different at byte: %d\n"), i);
                    bMatch = FALSE;
                  }
                  pOriginalBuffer++;
                  pRetrievedBuffer++;
      ...
      void testParameterEncodingSupport( void )
     {
          ...
          if (pStatement)
          {
            ...
            if (pResultSet)
            {
              if (pResultSet->Next())
              {
                ...
                wxDateTime dateValue = pResultSet->GetResultDate(_("DateCol"));
                ASSERT(wxInvalidDateTime == dateValue);
      ...
      void testSingleResultNoResults()
      {
        ...
        if (m_pDatabaseLayer)
        {
          ...
          {
#endif  
          int intValue = m_pDatabaseLayer->GetSingleResultInt(_("SELECT * FROM table8 WHERE IntCol = 1;"), 1);
#ifndef DONT_USE_DATABASE_LAYER_EXCEPTIONS
          FAIL("DatabaseLayer error should have been thrown");
#else
          ...
          {
#endif  
          long longValue = m_pDatabaseLayer->GetSingleResultLong(_("SELECT * FROM table8 WHERE IntCol = 1;"), 1);
#ifndef DONT_USE_DATABASE_LAYER_EXCEPTIONS
          FAIL("DatabaseLayer error should have been thrown");
#else
          ASSERT_EQUALS( DATABASE_LAYER_NO_ROWS_FOUND, m_pDatabaseLayer->GetErrorCode() );
...
#endif  
          wxMemoryBuffer buffer;
          void* blobValue = m_pDatabaseLayer->GetSingleResultBlob(_("SELECT * FROM table8 WHERE IntCol = 1;"), 4, buffer);
#ifndef DONT_USE_DATABASE_LAYER_EXCEPTIONS
          FAIL("DatabaseLayer error should have been thrown");
#else
          ASSERT_EQUALS( DATABASE_LAYER_NO_ROWS_FOUND, m_pDatabaseLayer->GetErrorCode() );
          ...
          {
#endif  
          double doubleValue = m_pDatabaseLayer->GetSingleResultDouble(_("SELECT * FROM table8 WHERE IntCol = 1;"), 2);
#ifndef DONT_USE_DATABASE_LAYER_EXCEPTIONS
          FAIL("DatabaseLayer error should have been thrown");
#else
          ASSERT_EQUALS( DATABASE_LAYER_NO_ROWS_FOUND, m_pDatabaseLayer->GetErrorCode() );
...
//  in the tests
      ...
      bool LoadSettings()
      {
        ...
        if (config.Read(_("/SQLite/databaseA"), &strDatabase))
        {
        ...
        {
          wxPrintf(_("SQLite database file not defined in settings file (databaseA=...)"));
          FAIL("SQLite database file not defined in settings file");
          return false;
        }

		if (config.Read(_("/SQLite/databaseB"), &strDatabase))
		{
			...
			m_pDatabaseLayer2 = new SqliteDatabaseLayer(strDatabase);
		}
		else
		{
			wxPrintf(_("SQLite database file not defined in settings file (databaseB=...)"));
			FAIL("SQLite database file not defined in settings file");
			return false;
		}
#elif USE_FIREBIRD
        ...
        if (haveServerInfo && haveUserInfo)
          m_pDatabaseLayer = new PostgresDatabaseLayer(strServer, strDatabase, strUser, strPassword);
...
// SQLite
        ...
        wxArrayString createView1Scripts;
        createView1Scripts.Add(_("--Skip Rewrite \n\nCREATE VIEW view1 AS SELECT table1.COLUMN1 AS col1 FROM table1;"));
        m_SqlMap[CREATE_VIEW1] = createView1Scripts;
		  
		  wxArrayString createTable10Scripts;
		  createTable10Scripts.Add(_("CREATE TABLE table10 (IntCol INTEGER PRIMARY KEY, StringCol VARCHAR(50));"));
		  m_SqlMap[CREATE_TABLE10] = createTable10Scripts;
		  
		  wxArrayString createTable11Scripts;
		  createTable11Scripts.Add(_("CREATE TABLE \"table11\" (IntCol INTEGER PRIMARY KEY, StringCol VARCHAR(50), ForeignCol INTEGER);"));
		  createTable11Scripts.Add(_("ALTER TABLE \"table11\" ADD CONSTRAINT \"fk_table11_ForeignCol_ID\" FOREIGN KEY ( \"ForeignCol\" ) REFERENCES \"table10\" ( \"IntCol\" );"));
		  m_SqlMap[CREATE_TABLE11] = createTable11Scripts;
		  
		  wxArrayString dropTable10Scripts;
		  dropTable10Scripts.Add(_("DROP TABLE table10"));
		  m_SqlMap[DROP_TABLE10] = dropTable10Scripts;
		  
		  wxArrayString dropTable11Scripts;
		  //dropTable11Scripts.Add(_("DROP CONSTRAINT fk_table11_ForeignCol_ID;"));
		  dropTable11Scripts.Add(_("DROP TABLE table11"));
		  m_SqlMap[DROP_TABLE11] = dropTable11Scripts;
